...
import { inventoryMasterApi, Manufacturer, companyApi, Company } from '../services/api';
...
export function ManufacturersPage() {
  ...
  const fetchManufacturers = async () => {
    try {
      setLoading(true);
      const data = await inventoryMasterApi.manufacturers.getAll();
      setManufacturers(Array.isArray(data) ? data : []);
    } catch (err: any) {
      const message = err.response?.data?.message || err.message || 'Failed to fetch manufacturers';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingManufacturer) {
        await inventoryMasterApi.manufacturers.update(editingManufacturer.id, formData);
        setToast({ message: 'Manufacturer updated successfully', type: 'success' });
      } else {
        await inventoryMasterApi.manufacturers.create(formData);
        setToast({ message: 'Manufacturer created successfully', type: 'success' });
      }
      setShowDrawer(false);
      resetForm();
      fetchManufacturers();
  ...
  const handleEdit = (manufacturer: Manufacturer) => {
    ...
    setFormData({
      ...
      companyId: (manufacturer as any).companyId || '',
    });
    setShowDrawer(true);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await inventoryMasterApi.manufacturers.delete(id);
      setToast({ message: 'Manufacturer deleted successfully', type: 'success' });
      fetchManufacturers();
    } catch (err: any) {
      const message = err.response?.data?.message || 'Failed to delete manufacturer';
      setToast({ message, type: 'error' });
    }
  };
  ...
  const filteredManufacturers = manufacturers.filter(
    (mfr) =>
      ...
      mfr.code.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-500">No manufacturers found</td>
                </tr>
              ) : (
                  ...
                  <tr key={manufacturer.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {manufacturer.website && (
                        <a href={manufacturer.website} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
                          {manufacturer.website}
                        </a>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">{manufacturer.code}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-gray-900">{manufacturer.contactPerson || '-'}</div>
                      <div className="text-sm text-gray-500">{manufacturer.email || manufacturer.contactEmail || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">
                      {manufacturer.city ? `${manufacturer.city}${manufacturer.country ? `, ${manufacturer.country}` : ''}` : '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs rounded-full ${manufacturer.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm text-gray-600">
                Showing {startIndex + 1}-{Math.min(endIndex, filteredManufacturers.length)} of {filteredManufacturers.length}
              </span>
            </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <label className="flex items-center cursor-pointer">
                      ...
                      />
                      <span className="ml-2 text-sm text-gray-700">Active manufacturer</span>
                    </label>
                  </div>
                </div>
